...
git add README.md docs .gitignore abdi_mlops_project/resources/README.md
...
- **`abdi_mlops_project-tests-ci.yml`**:<br>
  - **[CI]** Performs unit and integration tests<br>
    - Triggered on PR to main
- **`abdi_mlops_project-bundle-cicd.yml`**:<br>
  - **[CI]** Performs validation of Databricks resources defined under `abdi_mlops_project/resources`<br>
...
- You must be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://adb-1037601593204228.8.azuredatabricks.net#setting/accounts) and
  [prod workspace admin console](https://adb-1037601593204228.8.azuredatabricks.net#setting/accounts). If
...
1. Create a new variable group called `abdi_mlops_project variable group` defining the following secret variables, for more details [here](https://learn.microsoft.com/en-us/azure/devops/pipelines/library/variable-groups?view=azure-devops&tabs=classic#create-a-variable-group):
...
1. Update pipeline YAML files to use service connections rather than pipeline variables:
   - First, remove any lines where the environment variables are set in tasks in `abdi_mlops_project-tests-ci.yml` or `abdi_mlops_project-bundle-cicd.yml` files. Specifically, any lines where the following env vars are used: `PROD_AZURE_SP_TENANT_ID`, `PROD_AZURE_SP_APPLICATION_ID`, `PROD_AZURE_SP_CLIENT_SECRET`, `STAGING_AZURE_SP_TENANT_ID`, `STAGING_AZURE_SP_APPLICATION_ID`, `STAGING_AZURE_SP_CLIENT_SECRET`
...
Follow the instructions in [abdi_mlops_project/resources/README.md](../abdi_mlops_project/resources/README.md) to deploy ML resources
and production jobs.
...
[ML resource config guide](../abdi_mlops_project/resources/README.md)  to propose, test, and deploy
